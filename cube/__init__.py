...
def check_dependencies():
	...
	print (" = Optional check:")
	for dep, path in Config.OPTIONAL.items():
		if not os.path.exists(path):
			print(path, "not found; will not be used")
			Config.OPTIONAL[dep] = None
		elif os.path.getsize(path)==0:
			print(path, "is empty; will not be used")
			Config.OPTIONAL[dep] = None
	print ("\tOptional ok")
